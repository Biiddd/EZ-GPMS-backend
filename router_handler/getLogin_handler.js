...
const isCorrect = (req, res) => {
    ...
    db.query(query, [user_id], async (err, rows) => {
        if (err) {
            ...
            return res.status(500).json({ status: "未知错误请重试" });
        }

        if (rows.length === 0) {
            // 用户名不存在
            logger.warn("用户名不存在");
            return res.status(401).json({ status: "用户名不存在" });
        }
        ...
        if (password==rows[0].password) {
            ...
            if (rows[0].type === "老师") {
                ...
                db.query(teacherQuery, [user_id], (err, teacherRows) => {
                    ...
                    // 返回带有教师类型的信息
                    logger.info("教师登录成功");
                    return res.json({ status: "teacher success", teacher_type: teacherRows[0].teacher_type });
                });
            } else {
                // 普通用户登录成功
                logger.info("登录成功");
            ...
            // 密码不匹配
            logger.warn("密码不匹配");
            return res.status(401).json({ status:  "用户名或密码错误" });
        }
    });
};
